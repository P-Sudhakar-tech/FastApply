"""Benchmark: pandas .apply() vs .turbofastapply() vs swifter's .apply() on 1,000 rows.

Optional — requires swifter, which is NOT a package dependency (it pulls
in dask, tqdm, etc. that turbofastapply itself has no use for):
...
script for the plain pandas-vs-turbofastapply comparison and for why only the
...
def main():
    ...
    turbofastapply_times = timed(lambda: numbers.turbofastapply(func))
    try:
        ...
        print(
            ...
            "not a turbofastapply bug: swifter 1.4.0 calls a Series.apply() kwarg that "
    ...
    print(f"  .turbofastapply()      {t_med:8.4f} ms  ({p_med / t_med:.2f}x vs pandas)")
    ...
    print(
        ...
        "layer built with much larger data in mind, while turbofastapply's fast path\n"
