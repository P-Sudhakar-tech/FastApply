...
from turbofastapply import _turbofastapply, decide
...
def test_int_series_with_whole_coefficients_uses_int64_path_not_float():
    ...
    s = pd.Series(np.arange(LARGE_N, dtype="int64"))
    with mock.patch.object(_turbofastapply, "affine_i64", wraps=_turbofastapply.affine_i64) as spy_i64, \
         mock.patch.object(_turbofastapply, "affine_f64", wraps=_turbofastapply.affine_f64) as spy_f64:
...
def test_string_series_never_engages_numeric_fast_path():
    ...
    pd.testing.assert_series_equal(s.turbofastapply.apply(str.upper), s.apply(str.upper))
